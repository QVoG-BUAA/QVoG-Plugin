...
//        System.out.println(res);
    ...
    public static boolean C2Graph(String toolsPath, String targetPath,String fileName) throws IOException, InterruptedException {
        ...
        String C2Path = toolsPath + filePathSplit + "c2graph" + filePathSplit;
        String version = System.getProperty("os.name");
        boolean res = move(C2Path + "input" + filePathSplit, targetPath + filePathSplit + fileName);
        if(!res)
        {
            return false;
        }
        ...
        String GraphPath = targetPath + filePathSplit + "C2Graph" + sdf.format(System.currentTimeMillis()) + ".log";
        boolean result;
        if(version.toLowerCase().contains("windows"))
        {
            processBuilder.command("cmd.exe", "/c", cmd);

        } else if (version.toLowerCase().contains("linux") || version.toLowerCase().contains("mac")) {
        ...
        res = delete(C2Path + "input" + filePathSplit + fileName);
        if(!res)
        {
            return false;
        }
    ...
    {
        String filePathSplit = File.separator;
        String QueryPath = toolsPath + filePathSplit + "Query" + filePathSplit;
        String version = System.getProperty("os.name");
    ...
    public static boolean Java2Graph(String toolsPath, String targetPath, String fileName) throws IOException, InterruptedException {
        String filePathSplit = File.separator;
        String JavaPath = toolsPath + filePathSplit + "java2graph" + filePathSplit;

        boolean res = move(JavaPath + "input" + filePathSplit, targetPath + filePathSplit + fileName);
        if(!res)
        {
            return false;
        }
        ...
        ProcessBuilder processBuilder = new ProcessBuilder();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");
        String GraphFile = targetPath + filePathSplit + "Java2Graph" + sdf.format(System.currentTimeMillis()) + ".log";
        boolean result;
        if(version.toLowerCase().contains("windows"))
        {
            processBuilder.command("cmd.exe", "/c", cmd);
        ...
        res = delete(JavaPath + "input" + filePathSplit + fileName);
        if(!res)
        {
            return false;
        }
    ...
    public static boolean move(String filePath, String targetFilePath) throws IOException, InterruptedException {
        ...
        if(version.toLowerCase().contains("windows"))
        {
            String cmd = "copy /y " + targetFilePath + " " + filePath;
            processBuilder.command("cmd.exe", "/c", cmd);
